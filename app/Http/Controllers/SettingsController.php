...
//        ]);
    ...
    {
//        if (!helpers::checkPermission('setting_system')) {
//            return response()->json(['message' => "No tiene permisos"], 406);
//        }
...
//        $Currencies = Currency::where('deleted_at', null)->get(['id', 'name']);
        $clients = client::where('deleted_at', '=', null)->pluck('name','id');
//        $sms_gateway = sms_gateway::where('deleted_at', '=', null)->get(['id', 'title']);

        //get warehouses assigned to user
        $warehouses = helpers::getWarehouses(auth()->user());

        Inertia::share('titlePage', 'Ajustes de sistema');
        return Inertia::render('Settings/system_settings',[
//        return response()->json([
            'settings' => $item,
