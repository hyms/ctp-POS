...
function Verified_paidAmount() {
    if (isNaN(payment.value.amount)) {
        ...
        if (
            parseFloat(payment.value.amount) >
            parseFloat(payment.value.received_amount)
        ) {
            payment.value.amount = 0;
        } else if (
            parseFloat(payment.value.amount) > parseFloat(GrandTotal.value)
        ) {
            payment.value.amount = 0;
        }
    }
                ...
                <v-card class="card-order" :loading="loading">
                    ...
                    <v-dialog max-width="600" v-model="dialogUpdateDetail">
                        <v-card>
                            ...
                            <v-card-text>
                                <v-form
                                    @submit.prevent="submit_Update_Detail"
                                    ref="formUpdateDetail"
                                >
                                    <v-row>
                                        <!-- Unit Price -->
