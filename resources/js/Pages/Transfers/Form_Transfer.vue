...
function Calcul_Total() {
  ...
  for (let index = 0; index < detailsForm.value.length; index++) {
    let tax = detailsForm.value[index].taxe * detailsForm.value[index].quantity;
    detailsForm.value[index].subtotal = parseFloat(
        detailsForm.value[index].quantity * detailsForm.value[index].Net_cost + tax
    );
    total.value = parseFloat(total.value + detailsForm.value[index].subtotal);
  }
  ...
  GrandTotal.value = parseFloat(
      total_without_discount + transferForm.value.TaxNet + transferForm.value.shipping
  );
          ...
          <v-row>
            ...
            <v-col cols="12">
              ...
              <v-table hover class="border rounded">
                ...
                <tr class="bg-secondary">
                  ...
                  <th class="text-white text-center">{{ labels.transfer_detail.product }}</th>
                  <th class="text-white text-center">{{ labels.product.cost }}</th>
                  <th class="text-white text-center">{{ labels.transfer_detail.stock }}</th>
                  <th class="text-white text-center">{{ labels.transfer_detail.qty }}</th>
                  <!--                                        <th class="text-white text-center">{{$t('Discount')}}</th>-->
                  <!--                                        <th class="text-white text-center">{{$t('Tax')}}</th>-->
                  <th class="text-white text-center">{{ labels.transfer_detail.sub_total }}</th>
                  <th class="text-white text-center"></th>
                </tr>
                </thead>
                <tbody>
                <tr v-if="detailsForm.length <=0">
                  <td colspan="7">{{ labels.no_data_table }}</td>
                </tr>
                <tr v-for="detail in detailsForm">
                  <td>{{ detail.detail_id }}</td>
                  <td>
                    <v-chip color="primary" size="small">{{ detail.code }}</v-chip>
                    {{ detail.name }}
                    <!--                    <i @click="Modal_Updat_Detail(detail)" class="i-Edit"></i>-->
                  </td>
                  <td>{{ currency }} {{ helper.formatNumber(detail.Net_cost, 2) }}</td>
                  <td>
                    <v-chip color="default" size="small">{{ detail.stock }} {{ detail.unitPurchase }}</v-chip>
                  </td>
                  <td>
                    <div class="quantity">
                      <v-text-field
            ...
            <v-col cols="12" md="3" offset-md="9">
              <v-table hover>
                ...
                <tr>
                  <td>
                    <span class="font-weight-bold">{{ labels.transfer.GrandTotal }}</span>
                  </td>
                  <td>
